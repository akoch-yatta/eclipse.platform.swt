...
@Override Point computeSizeInPixels (int wHint, int hHint, boolean changed) {
	...
	if ((style & SWT.SEPARATOR) != 0) {
		...
		if ((style & SWT.HORIZONTAL) != 0) {
			width = DPIUtil.autoScaleUp(DEFAULT_WIDTH, getZoomFactor());  height = lineWidth * 2;
		} else {
			width = lineWidth * 2; height = DPIUtil.autoScaleUp(DEFAULT_HEIGHT, getZoomFactor());
		}
...
void wmDrawChildImage(DRAWITEMSTRUCT struct) {
	...
	gc.drawImage (image, DPIUtil.autoScaleDown(x, getZoomFactor()), DPIUtil.autoScaleDown(Math.max (0, (height - imageRect.height) / 2), getZoomFactor()));
