...
void drawItem (GC gc, long hTheme, RECT clipRect, boolean drawFocus) {
	...
	if (image != null) {
		...
		if (imageHeight > headerHeight) {
			gc.drawImage (image, DPIUtil.autoScaleDown(rect.left, getZoomFactor()), DPIUtil.autoScaleDown(rect.top + headerHeight - imageHeight, getZoomFactor()));
		} else {
			gc.drawImage (image, DPIUtil.autoScaleDown(rect.left, getZoomFactor()), DPIUtil.autoScaleDown(rect.top + (headerHeight - imageHeight) / 2, getZoomFactor()));
		}
...
public int getHeight () {
	checkWidget ();
	return DPIUtil.autoScaleDown(getHeightInPixels(), getZoomFactor());
}
...
public void setHeight (int height) {
	checkWidget ();
	setHeightInPixels(DPIUtil.autoScaleUp(height, getZoomFactor()));
}
