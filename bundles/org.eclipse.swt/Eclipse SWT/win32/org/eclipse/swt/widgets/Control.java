...
public Point computeSize (int wHint, int hHint, boolean changed){
	checkWidget ();
	wHint = (wHint != SWT.DEFAULT ? DPIUtil.autoScaleUp(wHint, getZoomFactor()) : wHint);
	hHint = (hHint != SWT.DEFAULT ? DPIUtil.autoScaleUp(hHint, getZoomFactor()) : hHint);
	return DPIUtil.autoScaleDown(computeSizeInPixels(wHint, hHint, changed), getZoomFactor());
}
...
public boolean dragDetect (MouseEvent event) {
	checkWidget ();
	if (event == null) error (SWT.ERROR_NULL_ARGUMENT);
	return dragDetect (event.button, event.count, event.stateMask, DPIUtil.autoScaleUp(event.x, getZoomFactor()), DPIUtil.autoScaleUp(event.y, getZoomFactor())); // To Pixels
}
...
public int getBorderWidth () {
	checkWidget ();
	return DPIUtil.autoScaleDown(getBorderWidthInPixels (), getZoomFactor());
}
...
public Rectangle getBounds (){
	checkWidget ();
	return DPIUtil.autoScaleDown(getBoundsInPixels (), getZoomFactor());
}
...
public Point getLocation () {
	checkWidget ();
	return DPIUtil.autoScaleDown(getLocationInPixels(), getZoomFactor());
}
...
public Point getSize (){
	checkWidget ();
	return DPIUtil.autoScaleDown(getSizeInPixels (), getZoomFactor());
}
...
public void redraw (int x, int y, int width, int height, boolean all) {
	checkWidget ();
	x = DPIUtil.autoScaleUp(x, getZoomFactor());
	y = DPIUtil.autoScaleUp(y, getZoomFactor());
	width = DPIUtil.autoScaleUp(width, getZoomFactor());
	height = DPIUtil.autoScaleUp(height, getZoomFactor());
...
public void setBounds (Rectangle rect) {
	...
	setBoundsInPixels(DPIUtil.autoScaleUp(rect, getZoomFactor()));
}
...
public void setLocation (Point location) {
	...
	location = DPIUtil.autoScaleUp(location, getZoomFactor());
...
public Point toControl (int x, int y) {
	checkWidget ();
	return DPIUtil.autoScaleDown(toControlInPixels(DPIUtil.autoScaleUp(x, getZoomFactor()), DPIUtil.autoScaleUp(y, getZoomFactor())), getZoomFactor());
}
...
public Point toControl (Point point) {
	...
	point = DPIUtil.autoScaleUp(point, getZoomFactor());
	return DPIUtil.autoScaleDown(toControlInPixels(point.x, point.y), getZoomFactor());
}
...
public Point toDisplay (int x, int y) {
	checkWidget ();
	return DPIUtil.autoScaleDown(toDisplayInPixels(DPIUtil.autoScaleUp(x, getZoomFactor()), DPIUtil.autoScaleUp(y, getZoomFactor())), getZoomFactor());
}
...
public Point toDisplay (Point point) {
	...
	if (point == null) error (SWT.ERROR_NULL_ARGUMENT);
	point = DPIUtil.autoScaleUp(point, getZoomFactor());
	return DPIUtil.autoScaleDown(toDisplayInPixels(point.x, point.y), getZoomFactor());
}
