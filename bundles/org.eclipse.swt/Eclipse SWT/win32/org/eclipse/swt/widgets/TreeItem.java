...
public Rectangle getBounds (int index) {
	checkWidget();
	return DPIUtil.autoScaleDown(getBoundsInPixels(index), parent.getZoomFactor());
}
...
public Rectangle getImageBounds (int index) {
	checkWidget();
	return DPIUtil.autoScaleDown(getImageBoundsInPixels(index), parent.getZoomFactor());
}
...
public Rectangle getTextBounds (int index) {
	checkWidget();
	return DPIUtil.autoScaleDown(getTextBoundsInPixels(index), parent.getZoomFactor());
}
...
public void setFont (Font font){
	...
	this.font = (font == null ? font : font.scaleFor(shell.getNativeZoomFactor()));
...
public void setFont (int index, Font font) {
	...
	if (oldFont == font) return;
	Shell shell = parent.getShell();
	cellFont [index] = font == null ? font : font.scaleFor(shell.getNativeZoomFactor());
	if (oldFont != null && oldFont.equals (font)) return;
	if (font != null) parent.customDraw = true;
	if ((parent.style & SWT.VIRTUAL) != 0) cached = true;
	/*
