...
import org.eclipse.swt.graphics.Region.Operation.*;
...
public void add (int[] pointArray) {
	...
	final Operation operation = new OperationWithArray(OperationType.ADD, Arrays.copyOf(pointArray, pointArray.length));
...
public void add (Rectangle rect) {
	...
	final Operation operation = new OperationWithRectangle(OperationType.ADD, new Rectangle(rect.x, rect.y, rect.width, rect.height));
	storeAndApplyOperationForAllHandles(operation);
}
...
public void add (int x, int y, int width, int height) {
	if (isDisposed()) SWT.error(SWT.ERROR_GRAPHIC_DISPOSED);
	final Operation operation = new OperationWithRectangle(OperationType.ADD, new Rectangle(x, y, width, height));
	storeAndApplyOperationForAllHandles(operation);
}
...
public void add (Region region) {
	...
	final Operation operation = new OperationWithRegion(OperationType.ADD, region);
	storeAndApplyOperationForAllHandles(operation);
}
...
public void intersect (Rectangle rect) {
	if (isDisposed()) SWT.error(SWT.ERROR_GRAPHIC_DISPOSED);
	if (rect == null) SWT.error(SWT.ERROR_NULL_ARGUMENT);
	final Operation operation = new OperationWithRectangle(OperationType.INTERSECT, new Rectangle(rect.x, rect.y, rect.width, rect.height));
	storeAndApplyOperationForAllHandles(operation);
}
...
public void intersect (int x, int y, int width, int height) {
	if (isDisposed()) SWT.error(SWT.ERROR_GRAPHIC_DISPOSED);
	final Operation operation = new OperationWithRectangle(OperationType.INTERSECT, new Rectangle(x, y, width, height));
	storeAndApplyOperationForAllHandles(operation);
	}
...
public void intersect (Region region) {
	...
	if (region == null) SWT.error(SWT.ERROR_NULL_ARGUMENT);
	if (region.isDisposed()) SWT.error(SWT.ERROR_INVALID_ARGUMENT);
	final Operation operation = new OperationWithRegion(OperationType.INTERSECT, region);
	storeAndApplyOperationForAllHandles(operation);
}
...
public void subtract (int[] pointArray) {
	if (isDisposed()) SWT.error(SWT.ERROR_GRAPHIC_DISPOSED);
	if (pointArray == null) SWT.error(SWT.ERROR_NULL_ARGUMENT);
	final Operation operation = new OperationWithArray(OperationType.SUBTRACT, Arrays.copyOf(pointArray, pointArray.length));
	storeAndApplyOperationForAllHandles(operation);
}
...
public void subtract (Rectangle rect) {
	if (isDisposed()) SWT.error(SWT.ERROR_GRAPHIC_DISPOSED);
	if (rect == null) SWT.error(SWT.ERROR_NULL_ARGUMENT);
	final Operation operation = new OperationWithRectangle(OperationType.SUBTRACT, new Rectangle(rect.x, rect.y, rect.width, rect.height));
	storeAndApplyOperationForAllHandles(operation);
}
...
public void subtract (int x, int y, int width, int height) {
	if (isDisposed()) SWT.error(SWT.ERROR_GRAPHIC_DISPOSED);
	final Operation operation = new OperationWithRectangle(OperationType.SUBTRACT, new Rectangle(x, y, width, height));
	storeAndApplyOperationForAllHandles(operation);
}
...
public void subtract (Region region) {
	...
	if (region == null) SWT.error(SWT.ERROR_NULL_ARGUMENT);
	if (region.isDisposed()) SWT.error(SWT.ERROR_INVALID_ARGUMENT);
	final Operation operation = new OperationWithRegion(OperationType.SUBTRACT, region);
	storeAndApplyOperationForAllHandles(operation);
}
...
public void translate (int x, int y) {
	if (isDisposed()) SWT.error(SWT.ERROR_GRAPHIC_DISPOSED);
	final Operation operation = new OperationWithPoint(OperationType.TRANSLATE, new Point(x, y));
	storeAndApplyOperationForAllHandles(operation);
}
...
public void translate (Point pt) {
	if (isDisposed()) SWT.error(SWT.ERROR_GRAPHIC_DISPOSED);
	if (pt == null) SWT.error(SWT.ERROR_NULL_ARGUMENT);
	final Operation operation = new OperationWithPoint(OperationType.TRANSLATE, new Point(pt.x, pt.y));
	storeAndApplyOperationForAllHandles(operation);
}
...
abstract class Operation {
	enum OperationType {
		ADD, SUBTRACT, INTERSECT, TRANSLATE
	}
	...
	void apply(long handle, int zoom) {
		...
		case ADD:
			...
			break;
		case SUBTRACT:
			subtract(handle, zoom);
			break;
		case INTERSECT:
			intersect(handle, zoom);
			break;
		case TRANSLATE:
			translate(handle, zoom);
			break;
		}
	}
...
private class OperationWithRectangle extends Operation {
	...
	OperationWithRectangle(OperationType operationType, Rectangle data) {
...
private class OperationWithArray extends Operation {
	...
	OperationWithArray(OperationType operationType, int[] data) {
		this.operationType = operationType;
		this.data = data;
	}
...
private class OperationWithPoint extends Operation {
	...
	OperationWithPoint(OperationType operationType, Point data) {
		this.operationType = operationType;
		this.data = data;
	}
...
private class OperationWithRegion extends Operation {
	...
	OperationWithRegion(OperationType operationType, Region data) {
		this.operationType = operationType;
		this.data = data;
	}
